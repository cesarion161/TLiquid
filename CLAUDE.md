...
- **Frameless + transparent + `always_on_top` + `visible_on_all_workspaces`**, which — combined with the macOS Accessory activation policy set in `lib.rs` — lets it float over other apps including fullscreen Spaces. **Transparent** (needs `app.macOSPrivateApi` + the `macos-private-api` tauri feature) so the optional **translucency** can show a frosted glass: `windows::apply_translucency` applies/clears an `NSVisualEffectView` (the `window-vibrancy` crate, translucent appearance-adaptive `UnderWindowBackground` material) per `config.ui.translucent` (default on; `set_translucency` persists + applies it live). Vibrancy gives a *real, persistent* desktop blur — CSS `backdrop-filter` only blurs a transparent window's backdrop transiently during resize, so it isn't used for the panel. The frontend also flips a `body.translucent` class that layers CSS over the vibrancy: a shared window-wide colour wash (`styles.css` `--tl-paint*`: percentage-positioned, `vmin`-sized `radial-gradient` blobs on `background-attachment: fixed`, so one set is shared across surfaces and scales without distorting) + light per-surface tints (`--tl-glass*`) for legibility; the overlay pane uses `backdrop-filter` (reliable in-document) to frost the translate view behind it. With translucency off, the opaque CSS background fills the transparent window so it looks solid; AppKit renders the material opaque under Reduce-Transparency (a CSS `prefers-reduced-transparency` fallback also exists). The slim titlebar (drag handle + bell/gear/back, no title text) is drawn in the UI (`.titlebar` with `data-tauri-drag-region`); dragging needs the `core:window:allow-start-dragging` capability.
