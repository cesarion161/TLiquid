...
- **Frameless + transparent + `always_on_top` + `visible_on_all_workspaces`**, which — combined with the macOS Accessory activation policy set in `lib.rs` — lets it float over other apps including fullscreen Spaces. **Transparent** (needs `app.macOSPrivateApi` + the `macos-private-api` tauri feature) so the optional **translucency** is a **window-level backdrop blur**: `windows::apply_translucency` calls the **private CoreGraphics API `CGSSetWindowBackgroundBlurRadius`** (the same mechanism Warp / iTerm2 / Alacritty use) per `config.ui.translucent` (default on; `set_translucency` persists + applies it live). The private API is required because `NSVisualEffectView` materials (`window-vibrancy`) have a *fixed* blur — this one is **tunable** (radius is a number). It rules TLiquid out of the App Store, which is fine — we're direct-distribution. `show_panel` re-calls `apply_translucency` since `NSWindow.windowNumber` is `0` until first show. **Corner shape tracks translucency**: the CGS blur fills the *rectangular* window frame and can't be clipped to a rounded shape (a layer mask only clips content, not the window-server blur), so rounded corners while translucent leave a blurred-desktop halo in the corners. So the panel is **square when translucent** (matches the blur — no halo) and **rounded when opaque** — `windows::panel_corner_radius` picks the native radius and the `.panel` CSS mirrors it via `body.translucent { border-radius: 0 }`; `round_window_corners` re-applies it on show / translucency-toggle / theme-change (an appearance change rebuilds the content layer and drops the mask). It also `invalidateShadow`s so the drop shadow hugs whichever shape. The frontend flips a `body.translucent` class layering over the blurred backdrop: a **panel tint** (`--tl-panel-bg`, alpha = "window opacity"), a shared colour wash (`styles.css` `--tl-paint*`: percentage-positioned, `vmin`-sized `radial-gradient` blobs on `background-attachment: fixed`, one set shared across surfaces, scales without distorting), and light per-surface tints (`--tl-glass*`) for box/pane definition. With translucency off, the opaque CSS background fills the transparent window so it looks solid; Reduce-Transparency is honored via a CSS `prefers-reduced-transparency` fallback. The slim titlebar (drag handle + bell/gear/back, no title text) is drawn in the UI (`.titlebar` with `data-tauri-drag-region`); dragging needs the `core:window:allow-start-dragging` capability.
- **Summon + dismiss (Spotlight-style)**: `show_panel` is the single entry point (there is no toggle) — `tray.rs` left-click, the tray menu's "Open"/"Settings…", and the translate hotkey (via `on_trigger`, for any outcome — captured text, a capture error, or no selection) all call it. Until the user drags the window, each summon anchors it under the tray icon via `position_under_tray` (reads the tray icon's screen `rect()` + the pure, unit-tested `panel_origin` clamping). The panel dismisses on **Esc** (App.svelte), **Cmd+W** (`CloseRequested` → hide, not destroy), and **Cmd+Q** (`menu.rs` repurposes it to hide, not quit — see below) — re-summon via the tray or the hotkey. **Losing focus no longer auto-hides** (it was too aggressive); `Focused(false)` only persists the window geometry. Real quit lives in the **tray menu** ("Quit T·Liquid"), which keeps the process alive after Cmd+Q so the global hotkey still works.
...
- `windows.rs` — the single panel window: create-hidden-at-startup, summon (`show_panel`)/hide, tray-anchored-then-draggable positioning + position persistence, geometry-save on blur (above). Also applies the saved appearance prefs: translucency, and the colour theme — `apply_theme`/`set_theme` map `ui.theme` ("light"/"dark"/"system") to the window appearance, which on macOS drives the webview's `prefers-color-scheme` so the existing `@media (prefers-color-scheme: dark)` CSS honours the choice (no separate dark selector).
- `menu.rs` — macOS app menu (mostly invisible for an accessory app; it owns key equivalents). Keeps the Edit-menu clipboard shortcuts the text fields need + Cmd+W, but replaces the predefined Quit with a **custom Cmd+Q that hides** instead of quitting — the predefined Quit sends AppKit `terminate:`, which bypasses Tauri's `ExitRequested` (so `prevent_exit` can't catch it). Real quit stays in the tray menu.
