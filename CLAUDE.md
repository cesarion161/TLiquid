...
- **Frameless + transparent + `always_on_top` + `visible_on_all_workspaces`**, which — combined with the macOS Accessory activation policy set in `lib.rs` — lets it float over other apps including fullscreen Spaces. **Transparent** (needs `app.macOSPrivateApi` + the `macos-private-api` tauri feature) so the optional **translucency** is a macOS **vibrancy** frosted glass: `windows::apply_translucency` applies/clears an `NSVisualEffectView` (the `window-vibrancy` crate, `HudWindow` material — the glassiest, so the most of what's behind shows through; dark-biased) per `config.ui.translucent` (default on; `set_translucency` persists + applies it live). Vibrancy is the only *stable* way to blur the desktop behind a transparent window (CSS `backdrop-filter` only blurs a transparent window's backdrop transiently during resize → flicker), but its blur is heavy + fixed and **the CSS tints must stay light or the glass goes opaque**. The frontend flips a `body.translucent` class layering over the frost: a shared colour wash (`styles.css` `--tl-paint*`: percentage-positioned, `vmin`-sized `radial-gradient` blobs on `background-attachment: fixed`, one set shared across surfaces, scales without distorting) + deliberately **light per-surface tints** (`--tl-glass*`; the panel is transparent so the frost shows). With translucency off, the opaque CSS background fills the transparent window so it looks solid; AppKit renders the material opaque under Reduce-Transparency (a CSS `prefers-reduced-transparency` fallback also exists). The slim titlebar (drag handle + bell/gear/back, no title text) is drawn in the UI (`.titlebar` with `data-tauri-drag-region`); dragging needs the `core:window:allow-start-dragging` capability.
