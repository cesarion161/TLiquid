...
| P1-008 | Improve macOS packaging, signing, and notarization | Phase 1 | FR-073, FR-074, FR-075 | dismissed | claude-opus-4.7 | 2026-05-27T15:10:26Z |  | Official macOS build process is documented; signing/notarization implemented if credentials exist; installer UX improved. | Can remain partially blocked by Apple developer account. **Dismissed/deferred 2026-05-27 (owner):** the signing+notarization *pipeline* is fully implemented (see notes below); the only remaining step — obtaining a **paid Apple Developer account** and producing a verified signed/notarized artifact — is deferred until TLiquid has real users (it costs money). Revisit then; builds stay unsigned (acceptable per FR-075, Gatekeeper bypass documented). Phase 1 completes without it. **Implemented (2026-05-27):** env-driven signing+notarization wired end-to-end — `bundle.macOS.hardenedRuntime:true` + new `src-tauri/Entitlements.plist` (JIT for the webview; no App Sandbox/AppleEvents); installer UX via `bundle.macOS.dmg` layout (windowSize + app/Applications icon positions); `scripts/build-macos.sh` official entry point that validates Apple cred consistency and reports signing/notarization mode; tag-triggered `.github/workflows/release.yml` that signs/notarizes only when repo secrets exist (works unsigned in forks); full guide in `docs/BUILD.md`; README signing section updated. `signingIdentity` left unset so default builds stay unsigned (FR-075 acceptable). Verified: config parses (`pnpm tauri build --no-bundle` green), `plutil -lint` on entitlements OK, JSON/YAML valid. **LEFT (needs owner — Apple Developer account):** obtain a Developer ID cert + App Store Connect API key, set the `APPLE_*` secrets/env, and verify a real signed+notarized artifact passes `spctl`/`stapler`. Kept in-progress per the credential blocker. |
...
1. **In-app updates** — **manual first (P2-007):** a "Check for updates" button +
   "Download & install", built and **thoroughly tested end-to-end before** the
   **automatic-polling (P2-013)** step (a default-ON "auto-check" toggle that polls
   GitHub every 3 hours, check-only — the user always clicks to install).
2. **Modern UI refresh (P2-012)** — optional Liquid-Glass translucency, a
   **resizable window** (min = current size; proportional inner layout), and
   **Settings/Notifications as fixed-width overlay panes** (not full-screen).

Hosted cloud/paid/diagnostics-backend tasks (P2-004/005/006/008) remain future and
money/infra-gated. **Windows is deferred to Phase 3+** (P2-001/002/003/009
reassigned — no Windows device; macOS-only release). **Translation history
(P2-010) is postponed indefinitely** (may not be done at all).
...
| P2-007 | Implement **manual** update check + click-to-install (macOS) | Phase 2 | FR-060, FR-061, FR-062, FR-063 | done | claude-opus-4.7 | 2026-05-27T21:00:00Z | 2026-05-27T21:02:28Z | Settings → Updates has a **"Check for updates"** button (works regardless of the auto-check toggle); on a successful check it shows the state (*Up to date* / *Update available: vX.Y.Z*), and when an update exists a nearby **"Download & install"** button becomes active; clicking it downloads the bundle, **verifies the signature, installs in place, and relaunches** (no manual `.dmg` reinstall). When an update is found, the **notification bell** gets a red dot and a notification *"New version vX.Y.Z is available!"* with a **"Download & install"** button in the same pane. | **DO THIS FIRST (2026-05-27, owner):** build + thoroughly test the manual check, download, install, and relaunch end-to-end before any automatic polling (P2-013). Mechanism: Tauri `updater` plugin → fetches **`latest.json` from GitHub Releases** + a full app bundle (`.app.tar.gz`); full-bundle replace (not binary patches). Uses the updater's **own minisign key** (separate from the deferred Apple Developer cert P1-008), so updates work while unsigned. Must not be silent/forced — the user always clicks to install. **Done (2026-05-27):** added `tauri-plugin-updater` (desktop) + new `updater.rs` (`PendingUpdate` managed state, `UpdateStatus`/`DownloadProgress`, pure-ish `check()` + `download_and_install()` over a `Channel`). Commands `check_for_update` / `download_and_install_update` (the latter `app.restart()`s into the new build, FR-063). Driven from Rust (not the JS plugin) so the background poll (P2-013) can reuse `updater::check`; no updater capability needed since we never `invoke` the plugin from JS. `tauri.conf.json`: `plugins.updater` (endpoint = `…/releases/latest/download/latest.json` + embedded minisign **pubkey**). `createUpdaterArtifacts` is kept OUT of the base config (with a pubkey set it would hard-require the signing key on every full build, breaking unsigned local + fork CI builds) and enabled via a partial-config override (`tauri.updater.conf.json`) only when `TAURI_SIGNING_PRIVATE_KEY` is present — addressed in the review follow-up. Frontend: `checkForUpdate`/`downloadAndInstallUpdate` wrappers + shared `lib/updates.ts` (`runUpdateInstall` channel/progress); `AboutSettings` Updates section (Check → Up to date / Update available vX.Y.Z + Download & install w/ progress); `App` owns `pendingUpdate`, lights the **bell** badge; `Notifications` shows the new-version alert + install. Signing wired into `release.yml` (`TAURI_SIGNING_PRIVATE_KEY[_PASSWORD]` env) + documented in `docs/BUILD.md` §6 (private key gitignored in `.tauri/`, draft-release-must-be-published caveat). Verified: clippy -D warnings, fmt, 78 tests, `pnpm check` 0, `pnpm build`. **Live end-to-end** (real release → check → download → install → relaunch) needs a published GitHub release + the private-key secret — owner action, part of P2-011 QA. |
...
| P2-011 | Phase 2 QA and release candidate | Phase 2 | All Phase 2 FRs | done | claude-opus-4.7 | 2026-05-27T21:55:00Z | 2026-05-27T22:05:00Z | macOS remains working; manual + automatic updates work; UI refresh/resizable/overlay-panes work; BYOK unrestricted mode remains intact. | macOS-only QA. **Scope updated (2026-05-27):** Windows deferred to Phase 3+; translation history postponed; hosted proxy/paid/diagnostics-backend are future. Phase 2 QA covers P2-007 (manual update) + P2-013 (auto-check) + P2-012 (UI/translucency/resizable/panes). **Done (2026-05-27):** Phase 2 QA recorded in `QA.md` (new P2-011 section). Automated gate all green in CI order: `pnpm check` 0 (126 files), `cargo fmt --check`, `cargo clippy -D warnings`, `cargo test` **79 passed** (+1 updates back-compat test), `pnpm tauri build --no-bundle` (release, transparent-window/vibrancy stack compiles). IPC surface **25 commands defined == registered == TS-wrapped** (zero mismatches; +3 since Phase 1: `check_for_update`, `download_and_install_update`, `set_translucency`). Privacy re-confirmed: the only new network call is the disclosed, opt-out, **GitHub-only** update check (separate surface from the `reqwest` provider layer; host-allowlist + confinement tests still pass); minisign private key gitignored, never committed. **Env limits (no interactive display / no published release / no signing secret):** the **live update loop** (check → download → minisign-verify → install → relaunch; needs `TAURI_SIGNING_PRIVATE_KEY` + a *published* GitHub release per `docs/BUILD.md` §6) and the **visual UI** (vibrancy + Reduce-Transparency, resize/min/proportional + size persistence, overlay-pane slide/scrim/Esc) require the owner's Mac — checklist in QA.md §7. All three in-scope tasks done + peer-reviewed. Phase 2 release candidate. |
| P2-012 | Modernize UI: translucency, resizable window, overlay panes (macOS) | Phase 2 | FR-046, PRD §19.1/§19.2/§19.3 | done | claude-opus-4.7 | 2026-05-27T21:30:00Z | 2026-05-27T21:52:00Z | (a) UI refreshed to feel modern (2026); the panel optionally uses macOS translucency/vibrancy ("Liquid Glass"-style) behind the content — toggleable, **no performance regression** (idle CPU/GPU + footprint per §13.1), respects Reduce-Transparency / Reduce-Motion (a11y). (b) **The window is resizable but cannot be made smaller than the current size** (current 360×270 becomes the min); inner fields/areas resize **proportionally with the same paddings and gaps** — layout must not break at larger sizes. (c) **Settings and Notifications become fixed-width overlay panes** that cover only part of the translate view when opened (not a full-screen takeover), so they don't look sparse when the window is large. | New (2026-05-27, owner). Translucency via Tauri window effects (NSVisualEffectView) — gate behind a setting; no idle-render/GPU cost when off; keep contrast/legibility. Resizable: set Tauri `minWidth/minHeight` = current size, `resizable: true`; verify drag-position persistence + tray-anchoring still work; the panes overlay (e.g. right-side or sliding) rather than replacing the whole body. **Done (2026-05-27):** (a) **Translucency** — window created `transparent(true)` (`app.macOSPrivateApi` + `macos-private-api` tauri feature); `windows::apply_translucency` applies/clears an `NSVisualEffectView` (`window-vibrancy` crate, `Sidebar` material, `Active`) per `config.ui.translucent` (default ON, serde-defaulted). `set_translucency` command persists + applies live; `AppearanceSettings.svelte` toggle; `App` flips a `body.translucent` class that makes `.panel`/`.titlebar` transparent (surfaces stay opaque for legibility) — degrades to solid when off, and AppKit honors Reduce-Transparency automatically; Reduce-Motion covered by the existing global transition guard. `save_settings` now also preserves the server-authoritative `ui.translucent` (like `startup`). (b) **Resizable** — `resizable(true)` + `min_inner_size`=360×270; size persisted independently of position (`SavedWindow` optional fields + `USER_SIZED`, back-compat with old `{x,y}` `window.json`); the source textarea + result both `flex:1` so the inner layout grows proportionally. (c) **Overlay panes** — translate is the always-present base; Settings/Notifications render in a right-aligned fixed-width (340px) `.overlay` that slides over part of it with a click-to-close scrim; **Esc** backs out of the overlay first (then hides). CLAUDE.md/README updated. Verified: clippy -D warnings, fmt, 79 tests, `pnpm check` 0, `pnpm build`, `pnpm tauri build --no-bundle`. **Live visual checks** (vibrancy look, resize behavior, pane overlay, Reduce-Transparency) need the owner's Mac with a display — part of P2-011 QA. **Post-iteration (2026-05-28):** translucency redesigned with owner in the loop. Switched from `NSVisualEffectView` (fixed-material vibrancy) to the **private `CGSSetWindowBackgroundBlurRadius`** CoreGraphics API — same mechanism Warp/iTerm2/Alacritty use — for a **tunable** window-level backdrop blur (radius=30). `--tl-panel-bg`/`--tl-glass*` tints layer over it (panel `.65`, text-box pure white/black at `.55`, panes pure white/black at `.80`). Native rounded corners (`10px`) via `setCornerRadius`/`masksToBounds` on the NSWindow content-view layer — keeps OS blur+shadow clipped to the rounded shape. Colour blobs (the earlier `--tl-paint*`) removed; will redesign later. Misc UI: thinner body padding (`4px 8px 8px`); Translate button gets `min-width` so its label swap doesn't shift the row; input `::placeholder` colour matches the output empty-state hint; output empty-state font-size matches the input placeholder. `window-vibrancy` dep dropped, `objc2` added (for the `windowNumber`/contentView selectors). Note: the CGS API is **private** (TLiquid is direct-distribution → fine; not App-Store-compatible). |
| P2-013 | Implement **automatic** update-check polling (macOS) | Phase 2 | FR-058, FR-059 | done | claude-opus-4.7 | 2026-05-27T21:05:00Z | 2026-05-27T21:28:58Z | A Settings → Updates toggle **"Automatically check for new updates"**, **default ON**. When ON, the app polls GitHub `latest.json` for a newer version **every 3 hours** (and on startup); the automatic part is **check-only — it never auto-downloads or installs**, the user still clicks "Download & install" (P2-007). A found update lights the **bell** (red dot + the "New version available" notification). When the toggle is OFF, no background polling happens, but the manual "Check for updates" button (P2-007) still works. | **DEPENDS ON P2-007 — only start after manual update check + install are built and verified (owner, 2026-05-27).** Poll interval = 3h. Non-silent/non-forced. This is the disclosed change to the Phase-0/1 "no automatic update checks" promise (FR-056) — document it; the toggle lets users opt out. **Done (2026-05-27):** `config::Updates { auto_check }` (default true, serde-defaulted + forward-compat test) added to `Settings`. `updater::start_auto_check()` (spawned in `lib.rs`, desktop) reuses `updater::check`: 10s startup delay, then every 3h (`POLL_INTERVAL`), re-reads `auto_check` each tick (off-toggle stops it without restart), and on a found update emits `update-available` to the panel — check-only, never installs. New direct dep `tokio` (time feature; already in-tree via reqwest) for the sleep. Frontend: `App` listens for `update-available` → sets `pendingUpdate` (lights bell + shows in Notifications). `AboutSettings` refactored so the **available-update UI is driven by the shared `pendingUpdate`** (so a background-found update shows Download & install without a manual click) + the **"Automatically check for new updates"** toggle (`settings.updates.autoCheck`, persisted via the shared `persist()`); `Settings` threads `update`/`settings`/`onChange` through. README Privacy section + CLAUDE.md updated to disclose the change to the FR-056 no-auto-check promise (GitHub-only, no personal data, opt-out). Verified: clippy -D warnings, fmt, 79 tests, `pnpm check` 0, `pnpm build`. Live polling against a published release is part of P2-011 QA. |
