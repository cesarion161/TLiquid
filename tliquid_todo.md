...
| P0-002 | Implement macOS menu-bar app shell | Phase 0 | FR-004, FR-005, FR-006, FR-007, FR-008 | not-started |  |  |  | App runs as single instance; appears in macOS menu bar; left-clicking the tray toggles the tray-anchored panel (translate + settings views); can quit from the tray's right-click menu; avoids Dock presence while idle where feasible; app remains alive in background. | Do not implement Windows/Linux tray behavior in Phase 0. Single window: a frameless panel created hidden at startup, always-on-top + visible-on-all-workspaces so it floats over fullscreen apps, positioned under the tray icon. Auto-hide-on-blur is a follow-up (PRD §19.2 Future). |
| P0-003 | Implement Svelte UI foundation and visual system | Phase 0 | FR-003, FR-046 | not-started |  |  |  | Svelte UI shell exists as a single panel with translate and settings views (switched in-app via `$state`, not separate windows); frameless panel has a custom draggable titlebar with a gear/back control; follows light/dark system theme where feasible; uses minimal styling; primary action uses TLiquid accent color; UI is responsive and small. | Keep UI utility-like, not chat-like. One window only — do not add per-screen HTML entries/windows. |
...
| P0-010 | Implement translation orchestrator and prompt routing | Phase 0 | FR-014, FR-015, FR-018, FR-019, FR-020, FR-044 | not-started |  |  |  | Translation orchestrator accepts source text, routing mode, language settings, provider/model; builds primary-mode and explicit-target prompts; returns translation only; preserves formatting/code instructions; no translation text is stored locally by default; no TLiquid server calls exist. | Include primary/secondary auto-routing prompt behavior. Phase 0 is non-streaming: await the full provider response and return one TranslationResponse (streaming is P1-009). |
| P0-011 | Implement manual translation in the panel | Phase 0 | FR-011, FR-016, FR-017, FR-018 | not-started |  |  |  | The panel's translate view contains input field, target selector, Translate button, output field, Copy button, and Enter-to-copy behavior after translation; real configured LLM provider is called; provider/network errors are shown compactly. | The panel opens anchored under the tray icon (not a separate window). |
