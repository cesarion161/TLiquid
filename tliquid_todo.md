...
| P2-012 | Modernize UI: translucency, resizable window, overlay panes (macOS) | Phase 2 | FR-046, PRD §19.1/§19.2/§19.3 | not-started |  |  |  | (a) UI refreshed to feel modern (2026); the panel optionally uses macOS translucency/vibrancy ("Liquid Glass"-style) behind the content — toggleable, **no performance regression** (idle CPU/GPU + footprint per §13.1), respects Reduce-Transparency / Reduce-Motion (a11y). (b) **The window is resizable but cannot be made smaller than the current size** (current 360×270 becomes the min); inner fields/areas resize **proportionally with the same paddings and gaps** — layout must not break at larger sizes. (c) **Settings and Notifications become fixed-width overlay panes** that cover only part of the translate view when opened (not a full-screen takeover), so they don't look sparse when the window is large. | New (2026-05-27, owner). Translucency via Tauri window effects (NSVisualEffectView) — gate behind a setting; no idle-render/GPU cost when off; keep contrast/legibility. Resizable: set Tauri `minWidth/minHeight` = current size, `resizable: true`; verify drag-position persistence + tray-anchoring still work; the panes overlay (e.g. right-side or sliding) rather than replacing the whole body. |
