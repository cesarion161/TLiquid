...
| P2-013 | Implement **automatic** update-check polling (macOS) | Phase 2 | FR-058, FR-059 | not-started |  |  |  | A Settings → Updates toggle **"Automatically check for new updates"**, **default ON**. When ON, the app polls GitHub `latest.json` for a newer version **every 3 hours** (and on startup); the automatic part is **check-only — it never auto-downloads or installs**, the user still clicks "Download & install" (P2-007). A found update lights the **bell** (red dot + the "New version available" notification). When the toggle is OFF, no background polling happens, but the manual "Check for updates" button (P2-007) still works. | **DEPENDS ON P2-007 — only start after manual update check + install are built and verified (owner, 2026-05-27).** Poll interval = 3h. Non-silent/non-forced. This is the disclosed change to the Phase-0/1 "no automatic update checks" promise (FR-056) — document it; the toggle lets users opt out. |
